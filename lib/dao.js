...
exports.addProgramme = function(newProgramme, db, callback) {
  //Add validation :)
  db.collection('programmelist').findOne({fbId : newProgramme.fbId}, function(err, result) {
    if (err) console.log(err); //improve this
      if (result == null) {
        db.collection('programmelist').insert(newProgramme, function(err, result) {
